...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('SDK') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('SDK'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('SDK')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('SDK'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('SDK')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('SDK'), __DIR__ . '/views');
